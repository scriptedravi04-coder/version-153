...
/**
 * Filter locations based on search query with smart ranking
 */
export function searchLocations(query: string, limit: number = 15): Array<{
  ...
  if (!q) {
    // Return popular states and major metros
    const topPopular = [
      { name: "Pan India", type: "Region", country: "India", display: "Pan India (All Cities & States)" },
  ...
  // 2. Search States & Union Territories
