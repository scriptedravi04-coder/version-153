...
export default function AgreementSign({ thread, onClose, onSigned }) {
  ...
  const getRegisteredUserPhone = (u) => {
    ...
    if (!raw || !String(raw).trim()) {
      ...
      } catch (e) { ignored("AgreementSign:23", e); }
    }
  ...
  // Pre-filled from the registered number; editable when there is none.
  const [signerMobile, setSignerMobile] = useState(() => String(getRegisteredUserPhone(user) || '').replace(/\D/g, '').slice(-10));

  useEffect(() => {
    const p = getRegisteredUserPhone(user);
    if (p) setSignerMobile(String(p).replace(/\D/g, '').slice(-10));
  }, [user]);
  ...
  const handleSign = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post(`/campaign/threads/${thread.id}/sign`, { signer_mobile: mobile, signer_side: 'creator' });
      ...
      >
            ...
            >
              ...
              <div className="space-y-6 border-t border-amber-900/20 pt-8">
                ...
                <div className="bg-amber-900/[0.02] border border-amber-900/10 rounded-2xl p-6">
                  <label className="text-xs font-bold text-slate-600 block mb-1 font-mono">Your Mobile Number</label>
                  <input
                    type="tel"
                    inputMode="numeric"
                    required
        ...
        {step === 'agreement' && (
          <div className="p-6 border-t border-amber-900/10 bg-slate-100 flex items-center justify-between">
            ...
            <button 
              onClick={handleSign}
              disabled={thread.agreement_signed_creator || loading}
            ...
            >
              ...
              {thread.agreement_signed_creator ? 'DEED EXECUTED' : (loading ? 'EXECUTING...' : 'EXECUTE COLLABORATION AGREEMENT')}
            </button>
          </div>
        )}
      </motion.div>
